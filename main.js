...
class DuneHdRemote extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (this.config.enablePwa) {
      ...
      try {
        ...
        this.log.info(
          `PWA remote available at: http://<iobroker-host>:${pwaPort}/`,
        );
      } catch (err) {
  ...
  async onStateChange(id, state) {
    ...
    try {
      switch (channel) {
        case "control":
          await this._handleControl(name, state.val);
          break;
        case "navigation":
          await this._handleNavigation(name, state.val);
          break;
        case "media":
  ...
  async _handleControl(command, value) {
    ...
    switch (command) {
      ...
      case "prev":
        if (value) {
          await this.player.playbackAction("prev");
        }
        break;
      case "next":
        if (value) {
          await this.player.playbackAction("next");
        }
        break;
      case "rewind":
        if (value) {
  ...
  async _handleNavigation(command, value) {
    ...
    await this.setStateAsync(`navigation.${command}`, {
      val: false,
      ack: true,
    });
  }
    ...
    switch (command) {
      case "playUrl":
        if (value) {
          ...
          await this.setStateAsync("status.currentUrl", {
            val: value,
            ack: true,
          });
        }
  ...
  _scheduleNextPoll() {
    ...
    this._pollingTimer = this.setTimeout(
  ...
  async _updateStatus() {
    ...
    try {
      ...
      if (!connected) {
        this.log.debug(
          `Player responded but command_status=${s.command_status}`,
        );
        return;
      }
      ...
      const playerStatus =
        stateMap[s.player_state] || s.player_state || "stopped";
      await this.setStateAsync("status.playerStatus", {
        val: playerStatus,
        ack: true,
      });

      if (s.playback_position !== undefined) {
        await this.setStateAsync("status.position", {
          val: parseInt(s.playback_position) || 0,
          ack: true,
        });
      }
      if (s.playback_duration !== undefined) {
        await this.setStateAsync("status.duration", {
          val: parseInt(s.playback_duration) || 0,
          ack: true,
        });
      }
      // XML uses playback_volume, text/json uses volume
      const vol =
        s.playback_volume !== undefined ? s.playback_volume : s.volume;
      if (vol !== undefined) {
        await this.setStateAsync("status.volume", {
          val: parseInt(vol) || 0,
          ack: true,
        });
      }
      ...
      if (mute !== undefined) {
        await this.setStateAsync("status.mute", {
          val: mute === "1" || mute === true,
          ack: true,
        });
      }
      ...
      if (s.playback_caption !== undefined) {
        await this.setStateAsync("status.caption", {
          val: s.playback_caption,
          ack: true,
        });
      }
      if (s.playback_picture !== undefined) {
        await this.setStateAsync("status.picture", {
          val: s.playback_picture,
          ack: true,
        });
      }
      if (s.playback_state !== undefined) {
        await this.setStateAsync("status.playbackState", {
          val: s.playback_state,
          ack: true,
        });
      }
      if (s.playback_current_bitrate !== undefined) {
        await this.setStateAsync("status.bitrate", {
          val: parseInt(s.playback_current_bitrate) || 0,
          ack: true,
        });
      }
      if (s.playback_is_buffering !== undefined) {
        await this.setStateAsync("status.isBuffering", {
          val:
            s.playback_is_buffering === "1" || s.playback_is_buffering === true,
          ack: true,
        });
      }
      if (s["audio_track.0.lang"] !== undefined) {
        await this.setStateAsync("status.audioLang", {
          val: s["audio_track.0.lang"],
          ack: true,
        });
      }
      if (s.playback_video_width !== undefined) {
        await this.setStateAsync("status.videoWidth", {
          val: parseInt(s.playback_video_width) || 0,
          ack: true,
        });
      }
      if (s.playback_video_height !== undefined) {
        await this.setStateAsync("status.videoHeight", {
          val: parseInt(s.playback_video_height) || 0,
          ack: true,
        });
      }
      if (s.product_name !== undefined) {
        await this.setStateAsync("info.playerModel", {
          val: s.product_name,
          ack: true,
        });
      }
      if (s.firmware_version !== undefined) {
        await this.setStateAsync("info.firmwareVersion", {
      ...
      if (!this._wasDisconnected) {
        this.log.warn(
          `Player unreachable (${this.config.playerIP}:${this.config.playerPort}): ${err.message}`,
        );
        this._wasDisconnected = true;
      } else {
        this.log.debug(
          `Player still unreachable (${this.config.playerIP}): ${err.message}`,
        );
      }
      await this.setStateAsync("info.connection", { val: false, ack: true });
    }
  ...
  async _createObjects() {
    ...
    // navigation channel
    await this.extendObjectAsync("navigation", {
      type: "channel",
    ...
    // media channel
    await this.extendObjectAsync("media", {
      type: "channel",
      common: { name: "Media" },
      native: {},
    });
